...
class ChatApp:
    ...
    def send_message(self):
        ...
        if message:
            if predict_spam_or_ham_nb(message) == "spam" or predict_spam_or_ham_svm(message) == "spam":
                tk.messagebox.showerror("Error",
                                        "Your message is marked as spam so it cannot be sent. Please check your messages again.")
                return
        ...
        while running:
            try:
                ...
                if verify_ECC(client_public_key, signature, decrypted_message):
                    ...
                    # Kiểm tra spam trên máy nhận
                    if predict_spam_or_ham_nb(message) == "spam" or predict_spam_or_ham_svm(message) == "spam":
                        print(f"Message from {addr} is marked as spam and will not be displayed.")
                        tk.messagebox.showerror("Error",
                                                "This message is marked as spam so it cannot be sent.")
                        continue
